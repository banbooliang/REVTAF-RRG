...
class BaseTrainer(object):
    def __init__(self, model, criterion_cls, base_probs, metric_ftns, args, device, is_main_process,logger):
        self.logger = logger
        self.args = args
        self.model = model
        self.device = device
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            # path = os.path.join(self.checkpoint_dir, f'model_{epoch}.pth')
        ...
        if self.is_main_process:
            self.logger.info('Best results (w.r.t {})'.format(self.mnt_metric))
...
class Trainer(BaseTrainer):
    def __init__(self, model, criterion_cls, base_probs, metric_ftns, args, logger, train_dataloader, val_dataloader, test_dataloader, device, is_main_process):
        super(Trainer, self).__init__(model, criterion_cls, base_probs, metric_ftns, args, device, is_main_process, logger)
    ...
    def _train_epoch_blip(self, epoch):
        ...
        for batch_idx, (images, captions, cls_labels, clip_memory, global_txt, region_txt, region_image) in enumerate(self.train_dataloader):
            ...
            global_txt = global_txt.to(self.device)
            region_txt = region_txt.to(self.device)
            self.lr_scheduler.step(cur_epoch=epoch, cur_step=batch_idx)
            # print(images.shape, cls_labels.shape, clip_memory.shape, global_txt.shape, region_txt.shape)
            ...
            loss = loss_lm + self.args.cls_weight*loss_cls + self.args.align_weight*loss_align + self.args.rank_weight*loss_rank
            if batch_idx%10 == 0:
                print("{}/{} loss: {}, loss_lm: {}, loss_cls: {}, loss_align: {}, loss_rank: {}".format(batch_idx, len(self.train_dataloader), loss.item(), loss_lm.item(), self.args.cls_weight*loss_cls.item(), self.args.align_weight*loss_align.item(), self.args.rank_weight*loss_rank.item()))
                self.logger.info("{}/{} loss: {}, loss_lm: {}, loss_cls: {}, loss_align: {}, loss_rank: {}".format(batch_idx, len(self.train_dataloader), loss.item(), loss_lm.item(), self.args.cls_weight*loss_cls.item(), self.args.align_weight*loss_align.item(), self.args.rank_weight*loss_rank.item()))
    ...
    def eval_blip(self, epoch, log):
        ...
        with torch.no_grad():
            val_gts, val_res = [], []
            for batch_idx, (images, captions, cls_labels, clip_memory, region_txt, region_image) in enumerate(self.val_dataloader):
                images = images.to(self.device) 
                cls_labels = cls_labels.to(self.device)
                clip_memory = clip_memory.to(self.device)
                region_txt = region_txt.to(self.device)
                ...
                reports, cls_preds, cls_preds_logits = self.model.module.generate(images, clip_memory, region_txt, region_image, sample=False, num_beams=self.args.beam_size, max_length=self.args.gen_max_len, min_length=self.args.gen_min_len)
                ## logit adjustment
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images, captions, cls_labels, clip_memory, region_txt, region_image) in enumerate(self.test_dataloader):
                ...
                region_image = region_image.to(self.device)
                ground_truths = captions
                reports, _, _ = self.model.module.generate(images, clip_memory, region_txt, region_image, sample=False, num_beams=self.args.beam_size, max_length=self.args.gen_max_len, min_length=self.args.gen_min_len)
